...
public class ServerSend
{
    ...
    {
        _packet.WriteLength();
        for (int i = 1; i <= Server.serverData.maxPlayers; i++)
        {
    ...
    public static void SpawnPlayer(int _toClient, Player _player, Transform[] spawnPoints)
    {
        using (Packet _packet = new Packet((int)ServerPackets.spawnPlayer))
        {
            _packet.Write(_player.id);
            _packet.Write(_player.username);
            _packet.Write(_player.modelIndex);
            _packet.Write(_player.materialIndex);
            _packet.Write(0); // TEMP - For Now just use index 0 for starting gun

            int rand = Random.Range(0, spawnPoints.Length);
            _packet.Write(spawnPoints[rand].position);
            _packet.Write(spawnPoints[rand].rotation);
    ...
    public static void SpawnEnemy(Enemy _enemy, int _modelIndex)
    {
        using (Packet _packet = new Packet((int)ServerPackets.spawnEnemy))
        {
            

            SendTCPDataToAll(SpawnEnemy_Data(_enemy, _modelIndex, _packet));
        }
    }
    public static void SpawnEnemy(int _toClient, int _modelIndex,  Enemy _enemy)
    {
        using (Packet _packet = new Packet((int)ServerPackets.spawnEnemy))
        {
            SendTCPData(_toClient, SpawnEnemy_Data(_enemy, _modelIndex, _packet));
        }
    }

    private static Packet SpawnEnemy_Data(Enemy _enemy, int _modelIndex, Packet _packet)
    {
        _packet.Write(_enemy.id);
        _packet.Write(_modelIndex);
    ...
    //Gets a player's animation data and sends it to all players on server
    public static void PlayerAnimationData(Player _player, float[] _data)
    {
        using(Packet _packet = new Packet((int)ServerPackets.playerAnimationData))
        {
            _packet.Write(_player.id);
    ...
    //Gets a client video player status on button click and update for all users on server
    public static void VideoPlayerStatus(bool isPlaying, int currentIndex)
    {
        using(Packet _packet = new Packet((int)ServerPackets.videoPlayerStatus))
        {
            _packet.Write(isPlaying);
            _packet.Write(currentIndex);

            SendTCPDataToAll(_packet);
        }
    }

    public static void PlayerCosmetics(int _id, int modelIndex)
    {
        using(Packet _packet = new Packet((int)ServerPackets.playerCosmetics))
        {
            _packet.Write(_id);
